...
char View::output(){
	...
	cout << "image output begin" << endl;

	//intarray2bmp::intarray2bmp("output.bmp", &(data[0][0]),  Yres,  Xres,0, (pxSize * pxSize));
    outputToSdl(); 
    //outputToBmp();
     ...
     int max_value = pxSize * pxSize;
      ...
      for (int x=0; x<Xres; x++){
          for (int y=0; y<Yres; y++){
              ...
              sPixel(disp, x, y, SDL_MapRGB(disp->format, int(val), int(val), int(val)));
          }
      }
}
